...
OBJECTS = $(patsubst $(CURDIR)/sources/%.cpp, $(CURDIR)/bin/%.o, $(wildcard $(CURDIR)/sources/*.cpp))
DEPS = $(patsubst $(CURDIR)/bin/%.o, $(CURDIR)/Dependencies/%.d, $(OBJECTS))
TESTS = $(patsubst $(CURDIR)/sources/%_test.cpp, $(CURDIR)/bin/Test_%.out, $(wildcard $(CURDIR)/sources/*_test.cpp))
TARGETS = $(TESTS)

DOCS = $(patsubst $(CURDIR)/Docs/%.tex, $(CURDIR)/Docs/%.pdf, $(wildcard $(CURDIR)/Docs/*.tex))
...
$(CURDIR)/bin/Test_SQLiteManager.out: $(CURDIR)/bin/SQLiteManager_test.o $(CURDIR)/bin/SQLiteManager.o
	$(COMPILER) $(LIBS) -lboost_unit_test_framework -lboost_filesystem -lboost_system -lsqlite3 -o$@ $^
...
$(CURDIR)/bin/Test_JSONParser.out: $(CURDIR)/bin/JSONParser_test.o $(CURDIR)/bin/JSONParser.o
...
.PHONY: run_tests
run_tests:
	 $(foreach test, $(TESTS), echo "Running $(test)..."; $(test) --report_level=detailed;)
